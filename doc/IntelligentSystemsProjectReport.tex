...
\chapter{Fuzzy Inference System}
	\noindent In order to develop a fuzzy inference system to estimate the values of arousal an analysis of the 3 best features selected by running 5 times the sequentialfs algorithm (the reason behind the choice of this number is the same explained in the introduction). This analysis is performed by the script \texttt{matlab/analysis\_for\_fis.m}. The other files of this task are:
	\begin{itemize}
...
\end{itemize}
	...
	\noindent Then we have to check how many samples that has a low/medium/high output correspond to the different values of a particular feature. In order to do so we have to fix the meaning of low, medium and high output.
...
\newpage
	...
	\noindent In order to understand the reasoning behind the rule let make an example. We take into account the rule number 3 i.e. \texttt{"If (Feature24 is high) then (arousal is low)"}, we can see that we can infer this rule analyzing the three graphs (figures \ref{img: hist_example1}, \ref{img: hist_example2}, \ref{img: hist_example3}), in fact the arousal feature 24 is high more or less for values greater than 0.5 (figure \ref{img: input1_fuzzy}) and for this values there are no sample for what deal with the medium and high outputs instead there are values for low outputs. This means that if the feature 24 has values more or less greater or equal than 0.5 (so it assume high values) then the arousal is low. So we have inferred the fuzzy rule 3.
